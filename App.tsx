...
import CctvVisionPanel from './components/vision/CctvVisionPanel';
...
import { PermanentKnowledgeService } from './lib/document/permanent-knowledge-service';
...
interface ChatAttachmentMeta {
  ...
  imageDataUrl?: string;
  /** Firebase Storage download URL (persisted, used for chat rendering). */
  storageUrl?: string;
}
...
function AppShell({ children }: { children: React.ReactNode }) {
  ...
  // The camera button opens native photo/video capture with `capture=environment`
  // for a normal device photo/video flow. The paperclip button
  // accepts ANY file type — extraction logic in lib/chat-attachments.ts pulls
  // text from PDF/DOCX/XLSX/CSV/PPTX/plain text, images go through Gemini
  // Vision/object detection, and videos are uploaded as Drive references.
  ...
  const handleChatImagePick = useCallback(() => {
    chatImageInputRef.current?.click();
  }, []);

  const handleChatImageCancel = useCallback(() => {
    if (pendingImage) {
  ...
  const handleChatImageChange = useCallback(
  ...
  const ingestChatFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        let visionText = '';
        let objectDetectionText = '';
        if (att.kind === 'image' && att.imageDataUrl) {
          visionText = await describeImage(att.imageDataUrl);
          try {
            const detection = await ObjectDetectionService.detectDataUrl(att.imageDataUrl, att.filename);
            useCctvVisionStore.getState().setLastFrame(att.imageDataUrl);
            useCctvVisionStore.getState().setLastResult(detection);
            objectDetectionText =
              `Object detection:\n${detection.summary}\nBoxes:\n` +
              detection.detections
                .map(item =>
                  `- ${item.label} (${Math.round(item.score * 100)}%) x=${Math.round(item.box.x)} y=${Math.round(item.box.y)} w=${Math.round(item.box.width)} h=${Math.round(item.box.height)}${item.threat ? ` threat=${item.threatReason}` : ''}`,
                )
                .join('\n');
          } catch (error) {
            console.warn('Image object detection skipped:', error);
          }
        }

        const knowledgeResult = await DriveKnowledgeService.ingestUploadedFile(file, {
          userRequest: 'Text chat upload for Beatrice knowledge base.',
        }).catch(error => ({
          success: false,
          message: error instanceof Error ? error.message : 'Knowledge upload failed.',
          memorySaved: false,
        }));

        // Upload images to Firebase Storage for persistence
        let storageUrl: string | undefined;
        if (att.kind === 'image' && att.imageDataUrl && currentUser) {
          try {
            const convId = getActiveConversation()?.id;
            storageUrl = await uploadImageToStorage(currentUser.uid, file, convId);
          } catch (err) {
        ...
        const knowledgeText = knowledgeResult.message
          ? `Knowledge storage:\n${knowledgeResult.message}`
          : '';
        const finalText = [
          promptBody,
          visionText ? `Vision read-out:\n${visionText}` : '',
          objectDetectionText,
          knowledgeText,
        ].filter(Boolean).join('\n\n');
  ...
  return (
    <main className="device-container" id="app">
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 10, height: '100%' }}>
              ...
              chatMessages.map((msg, i) => (
                <div key={i} className="msg-bubble"
                  ...
                  {msg.attachment?.imageDataUrl && (
                    <img src={msg.attachment.imageDataUrl} alt={msg.attachment.filename || 'Attached image'}
          ...
          <form onSubmit={async (e) => {
            ...
            if (pendingImage) {
              ...
              if (currentUser) {
                try {
                  storageUrl = await uploadImageToStorage(currentUser.uid, pendingImage.file);
              ...
              const meta: ChatAttachmentMeta = {
                ...
                imageDataUrl: pendingImage.previewUrl,
                ...(storageUrl ? { storageUrl } : {}),
              };
              URL.revokeObjectURL(pendingImage.previewUrl);
              setPendingImage(null);
              await sendChatMessage('[Image attached]', meta);
            } else {
            ...
            {pendingImage && (
              <div className="image-preview-pill" style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', background: 'rgba(255,255,255,0.03)', borderRadius: '12px', padding: '6px 8px', border: '1px solid rgba(255,255,255,0.08)' }}>
                <img src={pendingImage.previewUrl} alt="Preview" style={{ width: '36px', height: '36px', borderRadius: '8px', objectFit: 'cover' }} />
                <span style={{ fontSize: '12px', color: '#d1d5db', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{pendingImage.file.name}</span>
                <button type="button" onClick={handleChatImageCancel} title="Remove"
                  style={{ width: '24px', height: '24px', borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <>
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px', padding: '16px', marginTop: '16px' }}>
                  ...
                  <p style={{ fontSize: '11px', color: '#6b7280', marginBottom: '12px', lineHeight: 1.5 }}>
                    Upload images, videos, or documents to Beatrice's Drive-backed knowledge storage. Readable documents are also indexed into long-term memory.
                  </p>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        if (!file) return;
	                        try {
	                          const result = await DriveKnowledgeService.ingestUploadedFile(file, {
	                            userRequest: 'Settings upload for Beatrice knowledge base.',
	                          });
	                          alert(`✅ ${result.message || `"${file.name}" uploaded.`}`);
	                        } catch (err) {
                  ...
                  <div style={{ fontSize: '10px', color: '#6b7280', marginTop: '8px', lineHeight: 1.4 }}>
	                    Images and videos are stored in Drive. PDF, DOCX, XLSX, TXT, MD, CSV, and JSON files are also indexed when readable.
                  </div>
                </div>
              </>
            )}
      ...
      {(() => {
        ...
        const navItem = (target: string, icon: string, title = target.replace('view-', '')) => (
          <button
            key={target}
        ...
        const actionItem = (key: string, icon: string, title: string, onClick: () => void) => (
          <button
            key={key}
            onClick={onClick}
            title={title}
        ...
        return (
          <nav id="bottom-nav"
            ...
            {navItem('view-text', 'ph ph-chat-teardrop-text', 'Text chat')}
            <button
              onClick={() => navigateTo('view-voice')}
              title="Open voice assistant"
              style={{
                width: '56px', height: '56px', borderRadius: '50%',
                background: 'linear-gradient(to bottom right, #d946ef, #7e22ce)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: 'white', border: '4px solid #0a0a0a',
